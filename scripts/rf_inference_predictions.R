...
select(vessel_code, vessel_type, crms, pred_crms, ddss:sea_chest_gratings) %>% 
...
# LoF thresholds---------------
d1 %>% pr_curve(crms, other) %>%  filter(precision< 1) %>% top_n(1, precision)
...
d1 %>% pr_curve(crms, sea_chest_gratings) %>%  filter(precision< 1) %>% top_n(1, precision)
