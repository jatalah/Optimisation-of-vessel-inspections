...
# get taxa fro NZDS and Golders Passengers datasets--------------
...
# get taxa from Cawthron datasets -------
bind_rows(
  Caw17 = read_csv(
  ...
  select(
    vessel_code,
    `Visible taxa (select from list. e.g. Al, Sp, Se, Is etc.)`:`Continuous strips > 50 mm in width?`   ,-c(`Video notes:`:`Typical fouling rep image #?`)
  ) %>%
  filter(vessel_code %in% distinct(read_csv('tables/misclasified_vessels.csv'), vessel_code)$vessel_code) %>%
...
# caw21---
filter(
  read_csv('cleaned_data/caw21_raw.csv') %>% rename(vessel_code = "vessel"),
  vessel_code %in% mis$vessel_code
) %>%
  select(vessel_code,
         gooseneck:algae_patch_50mm_width,
         -mobiles_write_in,
         -area_32) %>%
